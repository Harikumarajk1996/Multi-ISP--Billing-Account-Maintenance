...
try{
    $id = isset($_POST['id']) ? (int)$_POST['id'] : 0; if ($id<=0) throw new Exception('Invalid id');
    $stmt = $pdo->prepare('SELECT * FROM invoices WHERE id = ? LIMIT 1'); $stmt->execute([$id]); $row = $stmt->fetch(); if (!$row) throw new Exception('Invoice not found');
    $up = $pdo->prepare('UPDATE invoices SET status = ?, paid_at = NOW() WHERE id = ?'); $up->execute(['paid',$id]);
    echo json_encode(['success'=>true]);
}catch(Throwable $e){ error_log('mark_paid: '.$e->getMessage()); echo json_encode(['success'=>false,'error'=>$e->getMessage()]); }
