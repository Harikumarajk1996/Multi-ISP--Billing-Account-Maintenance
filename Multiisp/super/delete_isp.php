...
try{
    ...
    // optional: check if plans reference this isp
    $stmt = $pdo->prepare('SELECT COUNT(*) AS cnt FROM plans WHERE isp_id = ?');
    $stmt->execute([$id]); $row = $stmt->fetch();
    if ($row && $row['cnt']>0) { echo json_encode(['success'=>false,'error'=>'ISP in use by plans']); exit; }
    $del = $pdo->prepare('DELETE FROM isps WHERE id = ?');
    $del->execute([$id]);
    echo json_encode(['success'=>true]);
}catch(Throwable $e){ error_log('delete_isp: '.$e->getMessage()); echo json_encode(['success'=>false,'error'=>$e->getMessage()]); }
