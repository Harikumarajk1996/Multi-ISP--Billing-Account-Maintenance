...
try{
    ...
    $stmt=$pdo->prepare("INSERT INTO isps (name,slug,description,contact_email,status,created_at) VALUES (?,?,?,?,?,NOW())");
    $stmt->execute([$name,$slug,$description,$contact,$status]);
    echo json_encode(['success'=>true]);
}catch(Exception $e){ error_log('add_isp: '.$e->getMessage()); echo json_encode(['success'=>false,'error'=>$e->getMessage()]); }
