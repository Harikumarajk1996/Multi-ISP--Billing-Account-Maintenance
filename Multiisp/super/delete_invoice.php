...
try{
    ...
    $del = $pdo->prepare('DELETE FROM invoices WHERE id = ?'); $del->execute([$id]);
    echo json_encode(['success'=>true]);
}catch(Throwable $e){ error_log('delete_invoice: '.$e->getMessage()); echo json_encode(['success'=>false,'error'=>$e->getMessage()]); }
